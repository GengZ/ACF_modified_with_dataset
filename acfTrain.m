...
if( exist(crDir,'dir') && stage==0 )
  ...
  % -------------------------------------------------------------
  %
	% use posImgDir as the base
	GtPath = '/Annotations'; ImgPath = '/JPEGImages';
	baseDir = opts.posImgDir;	
	subDir = dir(baseDir);
  ...
  count_file = 1;
	for i = 1 : length(subDir)
		...
		subFullPath = [baseDir '/' subDir( i ).name];
    sub = bbGt('getFiles', { [subFullPath ImgPath] [subFullPath GtPath] });
    if count_file == 1
      fss = sub;
    else
		  fss = [fss sub];
 ...
 % fs=bbGt('getFiles',fs);
 % change fs to fss
  nImg=size(fss, 2); assert(nImg>0);
  if(~isinf(n)), fss=fss(:,randperm(nImg)); end; Is=cell(nImg*1000,1);
  ...
  while( i<nImg && k<n )
    ...
    parfor j=1:batch, ij=i+j;
      I = feval(opts.imreadf,fss{1,ij},opts.imreadp{:}); %#ok<PFBNS>
      gt=[]; if(hasGt), [~,gt]=bbGt('bbLoad',fss{2,ij},opts.pLoad); end
